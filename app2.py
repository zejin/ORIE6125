...
def BS_testing(X1,X2,C=1.5):
    ...
    B_square = float(n)**2/(n+2)/(n-1) * ((la.norm(sigma_hat, 'fro')**2) - 1.0/n * (tr_sigma_hat)**2)


    var_M_Original = 2.0*n*(n+1)/float((n1*n2)**2) * B_square
    ...
    var_M_Modified = 2.0*n*(n+1)/float((n1*n2)**2) * Q_hat

    Z_modified = float(M_stat/np.sqrt(var_M_Modified))
  

    return (2*(1-norm.cdf(abs(Z_original)))), 2*(1-norm.cdf(abs(Z_modified)))
...
def CQ_testing(X1,X2,C=1.5):
    ...
    T_stat = (sum(np.sum(X1X1,axis=1))-sum(np.diag(X1X1)))/float(n1)/float(n1-1)+(sum(np.sum(X2X2,axis=1))-sum(np.diag(X2X2)))/float(n2)/float(n2-1) - sum(np.sum(X1X2,axis=1))*2/float(n1)/float(n2)
    
    s1 = 0
    for j in range(n1):
        for k in range(n1):
            if j!=k:
                rows = [i for i in range(n1) if not i in [j,k]]
                X1_bar_jk = np.mean(X1[:,rows],axis=1)
                s1 = s1+ np.dot(np.dot(X1[:,j].T,(X1[:,k]-X1_bar_jk)),np.dot(X1[:,k].T,(X1[:,j]-X1_bar_jk)))

    sigma1_F_norm_square = s1/float(n1)/float(n1-1)
    s2 = 0
    for j in range(n2):
        for k in range(n2):
            if j!=k:
                rows = [i for i in range(n2) if not i in [j,k]]
                X2_bar_jk = np.mean(X2[:,rows],axis=1)
                s2 = s2+ np.dot(np.dot(X2[:,j].T,(X2[:,k]-X2_bar_jk)),np.dot(X2[:,k].T,(X2[:,j]-X2_bar_jk)))

    sigma2_F_norm_square = s2/float(n2)/float((n2-1))

    sigma12_innerprod = np.sum(np.diag(np.dot(np.dot((X1.T-X1_bar).T,X1.T),np.dot((X2.T-X2_bar).T,X2.T))))/float((n1-1))/float((n2-1))

    #print np.dot((X2-X2_bar),X2.T)
